...
class App(threading.Thread):
    ...
    def startgame(self):
        ...
        lock.release()
        messagebox.showinfo('startgame', 'start game')


    def openlog(self):
        messagebox.showinfo('openlog', 'open log')
    ...
    def run(self):
        ...
        startgame = Button(controlFrame, text='START', command=self.startgame)
        openlog = Button(controlFrame, text='OPENLOG', command=self.openlog)
        startgame.grid(row=0, column=1, columnspan=2)
        openlog.grid(row=0, column=3, columnspan=2)
...
def drawStartScreen():  # 开始界面
    ...
    drawText('PORT 50006 IS WAITING FOR CONNECTION', font, TEXTCOLOR, screen, 6.5, 2)
...
if __name__ == '__main__':
    '''
    可选测试对象
    Red  ：BetaCat1.0 | Human | Socket |
    Blue ：Human | Demo | Socket '
    Human表示棋手为人类.
    '''
    Red = 'BetaCat1.0'
    Blue = 'Demo'
