...
SLEEPTIME = 0
FUNCTION = -1   # -1初始化设置playernum  0悬空等待  1每人运行3次   2
playernum = 0
currentnum = 0
start = 0
#================================================================
...
class App(threading.Thread):
    ...
    def setup(self):
        global playernum
        global FUNCTION
        if FUNCTION == -1 and currentnum != 0:
            # lock = Lock()
            # lock.acquire()
            # FUNCTION = -1
            # playernum = int(self.numsEntry.get())
            # lock.release()
            try: # 实现并不美观，主要是为了防止同步时
                self.sk.connect(('127.0.0.1', 50006))
                self.sk.sendall(('for(;;) pause();').encode('utf8')) #magic word to stop listening
            except Exception as ex:
                print('last player has been online, you don\' need setup')
            sleep(1)
    ...
    def knockedout(self):
        ...
        if FUNCTION != 0:
            ...
            return 'knocked out'
        if playernum < 8:
            messagebox.showinfo('KNOCKEDOUT', 'AT LEAST 8 PLAYERS')
            return 'knocked out'
    ...
    def quit(self):
        ...
        if FUNCTION == -1 or FUNCTION == 0:
            try: # 实现上并不美观，实际上是为了解决客户端连接阻塞状态而设置的
                if playernum == 0:
                    self.sk.connect(('127.0.0.1', 50006))
                self.sk.sendall(('for(;;) pause();').encode('utf8'))  # magic word to stop listening
            except Exception as ex:
                print('quit directly')
            messagebox.showinfo('QUIT', 'SURE?') # 为了线程同步设置function 需要延迟一会儿，不可取消
            ...
            for i in range(playernum - 1):
                for j in range(playernum - i - 1):
                    if (rank[j].wins < rank[j + 1].wins):
                        rank[j], rank[j + 1] = rank[j + 1], rank[j]
        for i in range(playernum):
            logger.info('rank : ' + str(i) + '  ' + rank[i].ID + ' wins ' + str(rank[i].wins))
...
游戏启动主界面服务器50006端口开始监听，主界面进入阻塞状态

SETUP：连接若干玩家后（最多40个，直接自动完成设置），不足40人时点击进行设置playernum，仅能设置一次，
设置完成后自动取消主界面阻塞状态，JUDGE, BATTLE, KNOCKEDOUT功能可以执行
如不足40人时，必须点击setup才能开始游戏
...
JUDGE 和 BATTLE 结束之后可以在目录下面读取最新的CSV格式比赛结果文件
